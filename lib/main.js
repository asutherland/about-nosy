...
NosyClient.prototype = {
  /**
   * Close the tab associated with us.
   */
  killTabAndDie: function() {
    // this may throw depending on the shutdown ordering.
    try {
      this.uiWorker.tab.close();
    }
    catch(ex) {
    }
  },
  ...
  onInterval: function() {
    try {
      var sampledAt = Date.now(),
          ...
          wireRep = this._memFrobber.processExplicitTree(explicitTree,
                      sampledAt),
          cpuTallies = null;
      if ($cpufrob.hasProbes)
      ...
      console.error("Exception during memfrobbing:", ex);
    }
  },
...
exports.main = function() {
  ...
  // - destroy the about:nosy mapping at unload time
  // The protocol does not automatically unregister, so we need to do so.
  // Unregistering removes the category entry for our about handler, so
  //  about:nosy will no longer exist or map to us.
  $unload.when(function() {
    nosyProtocol.unregister();
  });
  ...
  // - close any about:nosy tabs that are alive at unload-time
  $unload.when(function() {
    nosyClients.forEach(function(client) {
      client.killTabAndDie();
    });
  });
};
