...
MemTreeFrobber.prototype = {
  ...
  _processMainJSRuntime: function(runtimeNode, generation) {
    ...
    for (var iKid = 0; iKid < kids.length; iKid++) {
      ...
      var remainder, idx, compType = null, uri = null, addrStr = null;
      ...
      if (compName[0] === "[") {
        ...
        if (remainder[0] !== '0') {
          ...
          // no origin processing; chrome compartments/sandboxes are explicit
        ...
        //  in code.
        uri = computeOriginForUri(compName);
      }

      if (compartmentsByName.hasOwnProperty(compName)) {
        compData = compartmentsByName[compName];
        // type, uri, addrStr are derived from the name, so immutable
        // XXX _nMerged can change.
        compData.generation = generation;
      }
      else {
        compData = compartmentsByName[compName] = {
          statId: this._issueStatId(),
    ...
    for (compName in compartmentsByName) {
      compData = compartmentsByName[compName];
      if (compData.generation !== generation) {
        this._removedCompartments.push(compData);
