 ...
 *  away.  Until that time (or some other time when reporters are changed), we
 *  lack the ability to tell many things apart that get lumped together.  For
 *  example, layout shells for the same URL are beyond us, compartments are
 *  based on origin in ways that are opaque to us, etc.
 *
 *
 ...
 *      compartment has only a single global object.  Or something like that.
...
MemTreeFrobber.prototype = {
    ...
    for (var iNode = 0; iNode < nodes.length; iNode++) {
      ...
      if ((outerMatch = RE_WIN_TOP.exec(outerNode._name))) {
        ...
        if (outerWindowsById.hasOwnProperty(outerIdStr)) {
          ...
          outerData = outerWindowsById[outerIdStr] = {
            id: parseInt(outerIdStr),
            topId: topId,
            reportedTopId: topId,
            innerWindows: {},
            touched: generation,
          };
          this._addedOuterWindows.push(outerData);
        }
        ...
        for (var iKid = 0; iKid < outerNode._kids.length; iKid++) {
          ...
          if (!(innerMatch = RE_WIN_INNER.exec(innerNode._name)))
            continue;
          // - inner window
          var innerIdStr = innerMatch[1], innerUri = innerMatch[2], innerData;
          if (outerData.innerWindows.hasOwnProperty(innerIdStr)) {
            ...
            innerData = outerData.innerWindows[innerIdStr] = {
              ...
              uri: innerUri.replace(RE_BACKSLASH, '/'),
            };
            this._addedInnerWindows.push(topId);
  ...
  _inferClosedWindows: function(generation) {
    for (var outerIdStr in this._outerWindowsById) {
      ...
      if (outerData.generation !== generation) {
        this._removedOuterWindows.push(outerData);
        continue;
      }
      ...
      for (var innerIdStr in outerData.innerWindows) {
        ...
        if (innerData.generation !== generation) {
          this._removedInnerWindows.push(innerData);
   ...
   *  windows are displaying the same URI, they will get merged by the tree
   *  logic, which is just as well because we can't tell them apart right now.
   */
  _processLayout: function(layoutNode) {
  },
   ...
   */
  _processMainJSRuntime: function(runtimeNode) {
    ...
    for (var iKid = 0; iKid < kids.length; iKid++) {
      ...
      var compName = kid._name.slice(CMPTLEN, -1), remainder, idx,
          compType = null, uri = null, addrStr = null;
      // System compartment
      if (compName[0] === "[") {
        compType = 'sys';
        remainder = compName.substring(SYSCMPLEN);

        // gobble uri if present
        if (remainder[0] !== '0') {
          idx = remainder.lastIndexOf(',');
          uri = remainder.substring(0, idx);
        ...
        compType = 'anon';
        uri = compName.substring(MOZNULL_LEN);
      }
      else {
        compType = 'web';
        uri = compName;
  ...
  processExplicitTree: function() {
    ...
    var domNode = explicitTreeNode.findKid('dom'),
        winObjsNode = domNode.findKind('window-objects');
