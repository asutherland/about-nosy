 ...
 *
 * # How do interesting memory reporter paths get formatted (as of 2012/03/18)?
 *
 ...
 * - DOM/layout: Now living together in harmony, with a base at
 *    "explicit/dom/window-objects".  There are still excellent comments in
 *    nsWindowMemoryReporter.cpp.
 *   - "top(URL, id=NNN)" parent the inner-windows which are grouped by
 *      CATEGORY (see below).
 *   - "top(none)" parents closed windows that are still alive for some reason.
 *      It is the former "other" category and its children are inner-windows
 *      without any intermediary CATEGORY.
 *   - "window(URL)" are the inner-windows under the category.
 *   - URIs are, in order of preference: the document URI, the window's
 *      principal's URI, or "[system]" as a fallback which presumably should
 *      only happen for chrome windows.
...
MemTreeFrobber.prototype = {
   ...
   *
   * This is now a single-pass algorithm.  At the conclusion of our sweep, we
   *  know that no other windows should exist.  Previously categories were
   *  separated.  As a transitional thing, we call _inferClosedWindows
   *  ourselves.
   */
  _processDomWindows: function(nodes, generation) {
    ...
    for (var iNode = 0; iNode < nodes.length; iNode++) {
      ...
      // We are not a leak detection tool right now, ignore the "other" category
      if (outerNode._name.lastIndexOf("top(none)", 0) === 0)
        continue;

      // Everything should be a match, but be mildly future-resistant and ignore
      //  things that don't fit into our world view.
      if (!(outerMatch = RE_WIN_TOP.exec(outerNode._name)))
        continue;

      var outerUrl = outerMatch[1], outerIdStr = outerMatch[2],
          outerId = parseInt(outerIdStr), outerData;
      if (outerWindowsById.hasOwnProperty(outerIdStr)) {
        outerData = outerWindowsById[outerIdStr];

        if (outerData.url !== outerUrl) {
          this._modifiedOuterWindows.push({
            id: outerId,
            url: outerUrl,
          });
          outerData.url = outerUrl;
        }
      ...
      // -- categories
      for (var iCat = 0; iCat < outerNode._kids.length; iCat++) {
        var catNode = outerNode._kids[iCat];

        for (var iKid = 0; iKid < catNode._kids.length; iKid++) {
          var innerNode = catNode._kids[iKid];
          if (!(innerMatch = RE_WIN_INNER.exec(innerNode._name)))
            continue;

          // nb: we used to have a filter that checked innerNode._amount for
          //  being less than 1024 because of a bunch of zombie-esque windows
          //  that would sit around confusing us.  This has now been removed,
          //  but we may need to bring that code back.  These may now be the
          //  [system] windows.

          // -- inner window
          // Inner windows used to include an id, allowing us to detect
          //  navigation on their part.  The id no longer exists, so we can
          //  no longer detect such navigation.  In the event it comes back,
          //  pull some code out of revision control.
          var innerUrl = innerMatch[1], innerData;
         
          // System windows are dead-ish, let's just ignore them for now since
          //  they are fairly boring.
          if (innerUrl === '[system]')
            continue;

          if (outerData.innerWindows.hasOwnProperty(innerUrl)) {
            innerData = outerData.innerWindows[innerUrl];
  ...
  _inferClosedWindows: function(generation) {
    for (var outerIdStr in this._outerWindowsById) {
      ...
      for (var innerUrl in outerData.innerWindows) {
        var innerData = outerData.innerWindows[innerUrl];
        if (innerData.touched !== generation) {
          this._removedInnerWindows.push(outerData.id);
          this._removedInnerWindows.push(innerData.statId);
          // XXX verify deletion during key iteration is actually safe per spec
          delete outerData.innerWindows[innerUrl];
        }
      }
    }
  },
  ...
  _processLayout: function(layoutNode, generation) {
    ...
    for (var iKid = 0; iKid < kids.length; iKid++) {
      ...
      var mergeCount = kid._kids[0]._nMerged || 1;
      if (shellsByUrl.hasOwnProperty(shellUrl)) {
        shellData = shellsByUrl[shellUrl];
        shellData.generation = generation;
        // forget deltas for now; we are normalizing in the stat gathering
        /*
        ...
        */
        ...
        this._addedShells.push(shellData);
      }
      statistics.push(shellData.statId);
      // let's always log the proportional value for now so we can always
      //  tie its statistics in direct.
      statistics.push(kid._amount / shellData.count);
    }
  ...
  _processMainJSRuntime: function(runtimeNode, generation) {
    ...
    for (var iKid = 0; iKid < kids.length; iKid++) {
      ...
      var remainder, idx, compType = null, url = null, rawUrl = null,
          urlOrigin = null, extensionInfo = null, addrStr = null, idxComma;
      ...
      if (compartmentsByName.hasOwnProperty(compName)) {
        ...
        if (compName[0] === "[") {
          ...
          // XXX asuth jsprobe build magic
          // I have my own patched build that provides addresses for web
          //  compartments too.
          idxComma = compName.lastIndexOf(', 0x');
          if (idxComma !== -1) {
            addrStr = compName.substring(idxComma + 2);
            rawUrl = compName.substring(0, idxComma);
          }
          else {
            rawUrl = compName;
          }
          ...
          //  in code.
          url = computeOriginForUri(rawUrl);
          // Is this coming from an Extension's space?
          if (RE_RESOURCE.test(url)) {
            extensionInfo = ExtensionsKing.mapOriginToExtension(url, rawUrl);
          }
        }
  ...
  processExplicitTree: function(explicitTreeNode, sampleTS) {
    ...
    // - DOM + layout
    var winObjsNode = explicitTreeNode.findKid('window-objects');
    this._processDomWindows(winObjsNode._kids, generation);
